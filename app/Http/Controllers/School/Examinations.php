...
class Examinations extends Controller
{
    ...
    public function exam_schedule(Request $request)
    {
        ...
        return view('school.examinations.exam-schedule',compact('group','subject','subject_group','data'));
    }
...
// $data = ExamResult::orderBy('id','desc')->get();
        ...
        if($request['session'] && $request['exam_group'] && $request['class'] && $request['section'])
        {
            $data= DB::table('exam_results')
                ...
                // ->join('student_admission', 'exam_results.class', '=', 'student_admission.class')
        ...
        //         ->join('student_admission', 'exam_results.add_no', '=', 'student_admission.id')
        //         // ->join('student_admission', 'exam_results.class', '=', 'student_admission.class')
        //         ->select('student_admission.*','exam_results.id as res_id')->orderBy('exam_results.id','desc')
        //         ->get();
                // dd($data);
...
//                     $getSubcat =  DB::table('student_admission')
//                     ->join('add_class','student_admission.class','=','add_class.id')
//                     ->select('student_admission.*')
//                     ->orderBy('student_admission.id','desc')
//                     ->where('student_admission.class',$cid)
//                     ->get();
//                     $html='<option value="">Select Course</option>';
//                     foreach($getSubcat as $list){
//                     $html.='<option value="'.$list->id.'">'.$list->roll_no.'</option>';
//                     }
//                     echo $html;
//                     }
...
//                     $cid=$request->post('cid');
//                     $getSubcat =  Stu_Admission::where('student_admission.id',$cid)->get();
//                     $html='<option value="">Select Course</option>';
//                     foreach($getSubcat as $list){
//                     $html.='<option value="'.$list->id.'">'.$list->admission_no.'</option>';
//                     }
//                     echo $html;
//                     }
...
//                     $html='<option value="">Select Course</option>';
//                     foreach($getSubcat as $list){
//                     $html.='<option value="'.$list->id.'">'.$list->first_name. " ".$list->last_name.'</option>';
//                     }
//                     echo $html;
//                     }
...
$request->validate([
    'roll_no'=>'required|unique:exam_results',
        ...
        // dd($add);
        if($add->save()){
            return redirect()->route('SCHOOL.exam-result')->with('success','Add Successfully');
        }else{
            return back()->with('error','Error');
        }
    }
...
// dd($data);
    ...
    function Student_result_up(Request $request , $id)
    {
        ...
        if($add->update()){
            return redirect()->route('SCHOOL.exam-result')->with('success','Update Successfully');
        }else{
            return back()->with('error','Error');
        }
    }
