...
class FrontOffice extends Controller
{
    public function index(Request $request)
    {
        ...
        {
            $show = AddmissionEnquiry::orderBy('id','desc')->where('user_id',$ids)->where('source',$request['create'])->whereBetween('date', [$request['enquiry_from_date'], $request['enquiry_to_date']])->get();
            // dd($show);
        }else{
            $show = AddmissionEnquiry::orderBy('id','desc')->where('user_id',$ids)->get();
        }
        $class = Add_Class::orderBy('id','desc')->where('user_id',$ids)->get();
    ...
    {
        $ids = auth()->user()->id;
        $show = AddVisitor::orderBy('id','desc')->where('user_id',$ids)->get();
    ...
    {
        $ids = auth()->user()->id;
        $show = PhoneCall::orderBy('id','desc')->where('user_id',$ids)->get();
