...
class Academics extends Controller
{
    ...
    {
        $ids = auth()->user()->id;
        $class = Add_Class::orderBy('class','asc')->where('user_id',$ids)->get()->sortByDesc('class');
        $sec = Add_Section::orderBy('id','desc')->get();
        if ($request['class'] && $request['section']) {
            ...
            $show = Stu_Admission::orderBy('id','desc')->where('active_status',1)->where('user_id',$ids)->limit(100)->get();
        }
    ...
    public function promote_student_data(Request $request , $id)
    {
        ...
        if($add->update()){
            ...
            $info->section = $add['section'];
            $info->save();
            return back()->with('success','Promoted Student Successfully');
        }else{
            return back()->with('error','Error');
        }  
    }
